...
export async function setupClaCheck() {
  ...
  try {
    ...
    if (
      ...
      core.setFailed(
        `Committers of Pull Request number ${input.getPrNumber(context.issue.number)} have to sign the CLA 📝`
      )
    }
  } catch (err) {
...
async function createClaFileAndPRComment(
  ...
  throw new Error(
    `Committers of pull request ${input.getPrNumber(context.issue.number)} have to sign the CLA`
  )
}
